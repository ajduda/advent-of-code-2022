...
def bestPossible(time,room,valveEncoding,myvalves):
    if time < 0:
    ...
    closedValves = decodeValves(myvalves) - valveSet
    ...
    for valve in closedValves:
        ...
        tempValue = bestPossible(time-timeDiff,valve,tempEncoding,myvalves) + (timeDiff * myPressure)
...
best = 0
fullMask = (1 << len(scorers)) - 1
iterations = fullMask >> 1

for mask in range(0,iterations):
    elephantMask = fullMask - mask
    temp = bestPossible(26,'AA',0,mask) + bestPossible(26,'AA',0,elephantMask)
    if temp > best:
        best = temp
